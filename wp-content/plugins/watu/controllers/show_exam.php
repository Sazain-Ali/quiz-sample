...
if(!empty($exam->use_honeypot) and !empty($_POST['do'])) {
	if(@$_POST['h_app_id'] != '__' . md5('honeyforme' . $_SERVER['REMOTE_ADDR'])) die('WATU_CAPTCHA:::'.__('No answer to the verification question.', 'watupro'));	
}   
...
if(isset($_REQUEST['do']) and $_REQUEST['do']) { // Quiz Reuslts.
	...
	foreach ($questions as $qct => $ques) {
		...
		if(!$correct and !$is_empty) $num_wrong++;
